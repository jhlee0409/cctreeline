...
Most statuslines tell you about *one* repo. `treeline` adds a third line that, for **only the worktrees you actually edited in the current session**, shows each one's open PR numbers — a view nothing else in the ecosystem gives you.
...
There are excellent general-purpose statuslines — [ccstatusline](https://github.com/sirmalloc/ccstatusline), [claude-hud](https://github.com/jarrodwatts/claude-hud), [claude-powerline](https://github.com/Owloops/claude-powerline). `treeline` is **not** trying to beat them on breadth, themes, or a config TUI. It does one thing they don't:
...
- **Auto-binding weekly gauge.** One gauge that picks whichever weekly bucket is currently binding (per `/status`) and labels its model scope — instead of N fixed gauges. *(Caveat: the per-model scope relies on the OAuth usage API returning a `limits[]` array; where it doesn't, treeline falls back to a flat `7d` gauge. See [Limitations](#limitations).)*
...
```bash
git clone https://github.com/<you>/treeline
cd treeline
bash install.sh
```
...
`${XDG_CONFIG_HOME:-~/.config}/treeline/config` (shell key=value):

| Key | Values | Default | Meaning |
|---|---|---|---|
| `TREELINE_USAGE_GAUGES` | `1` / `0` | `1` | Read credentials + show 5h/weekly gauges |
| `TREELINE_WORKTREE_LINE` | `1` / `0` / `auto` | `auto` | Line 3 (`auto` = on inside a git repo) |
| `TREELINE_COLOR` | `auto` / `256` / `none` | `auto` | `auto` honors `NO_COLOR` and `TERM=dumb` |
| `TREELINE_GLYPHS` | `unicode` / `ascii` | `unicode` | Glyph set |
| `TREELINE_DENSITY` | `full` / `compact` | `full` | `compact` = ctx + 5h only |
| `TREELINE_REUSE_HUD_CACHE` | `auto` / `0` | `auto` | Reuse claude-hud's usage cache if present (avoids a duplicate API call) |

## How it works

The render path (`treeline.sh` with no args) only reads stdin JSON and cache files, so it's fast and offline. Two background refreshers keep the caches warm (single-flight locked, TTL'd):

- `treeline.sh --refresh-usage` — calls the OAuth usage API, writes `~/.cache/treeline/usage.json` (60s TTL)
- `treeline.sh --refresh-worktrees <cwd>` — `git worktree list` + `gh pr list`, writes `~/.cache/treeline/worktrees.json` (120s TTL)
